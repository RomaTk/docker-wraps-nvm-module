...
function main {
    echo "Starting NVM download script..."
    ...
    for what_to_download in "${VERSIONS[@]}"; do
        ...
        arch="${parts[1]}"
        version="${parts[2]}"
        echo "Installing NVM version: $version for OS: $os and Architecture: $arch"
        # Here you would add the commands to install the specific version of NVM
        ...
        echo "Downloading NVM version: $version"
        (wget -c "https://github.com/nvm-sh/nvm/archive/refs/tags/v${version}.tar.gz" -O "$file_name")
        [ $? -ne 0 ] && exit 1
